...
public class Tower{
    ...
    /**
     * Añade una taza normal a la torre con el tamaño indicado.
     * Este método mantiene compatibilidad con los ciclos anteriores.
    ...
    /**
     * Añade una nueva taza del tipo indicado a la torre.
     * @param type subtipo de taza a crear.
     * @param size tamaño del objeto.
     */
    public void pushCup(String type, int size) {
        StackItem newCup = createCupByType(type, size);
    
        if (newCup == null || !canAdd(newCup)) {
            ok = false;
            return;
        }
    
        newCup.prepareInsertion(items);
        items.add(newCup.getInsertionIndex(items), newCup);
        idCounter++;
        colorIndex++;
    ...
    public void popCup(){
        ...
        if (!lastItem.isCup() || !lastItem.canBeRemoved(items)) {
            ok = false;
            return;
        }
    ...
    public void removeCup(int i){
        ...
        if (item == null || !item.isCup() || !item.canBeRemoved(items)) {
            ok = false;
            return;
        }
    ...
    /**
     * Añade una tapa normal a la torre con el tamaño indicado.
     * Este método mantiene compatibilidad con los ciclos anteriores.
     * @param i tamaño del objeto.
     */
    ...
    /**
     * Añade una nueva tapa del tipo indicado a la torre.
     * @param type subtipo de tapa a crear.
     * @param size tamaño del objeto.
     */
    public void pushLid(String type, int size) {
        StackItem newLid = createLidByType(type, size);
    
        if (newLid == null || !newLid.canEnterTower(items) || !canAdd(newLid)) {
            ok = false;
            return;
        }
    
        newLid.prepareInsertion(items);
        items.add(newLid.getInsertionIndex(items), newLid);
        idCounter++;
    ...
    public void popLid(){
        ...
        if (!lastItem.isLid() || !lastItem.canBeRemoved(items)) {
            ok = false;
            return;
        }
    ...
    public void removeLid(int i){
        ...
        if (item == null || !item.isLid() || !item.canBeRemoved(items)) {
            ok = false;
            return;
        }
    ...
    /**
     * Crea una taza del subtipo indicado.
     * @param type subtipo solicitado.
     * @param size tamaño del objeto.
     * @return la taza creada o null si el tipo no existe.
     */
    private StackItem createCupByType(String type, int size) {
        int centerX = width / 2;
        String color = CUP_COLORS[colorIndex % CUP_COLORS.length];
    
        switch (type.toLowerCase()) {
            case "normal":
                return new Cup(idCounter, size, color, centerX, FLOOR_Y, size);
            case "opener":
                return new OpenerCup(idCounter, size, color, centerX, FLOOR_Y, size);
            case "hierarchical":
                return new HierarchicalCup(idCounter, size, color, centerX, FLOOR_Y, size);
            case "ghost":
                return new GhostCup(idCounter, size, color, centerX, FLOOR_Y, size);
            default:
                return null;
        }
    }
    ...
    /**
     * Crea una tapa del subtipo indicado.
     * @param type subtipo solicitado.
     * @param size tamaño del objeto.
     * @return la tapa creada o null si el tipo no existe.
     */
    private StackItem createLidByType(String type, int size) {
        int centerX = width / 2;
        String color = findColorForLid(size);
    
        switch (type.toLowerCase()) {
            case "normal":
                return new Lid(idCounter, size, color, centerX, FLOOR_Y, size);
            case "fearful":
                return new FearfulLid(idCounter, size, color, centerX, FLOOR_Y, size);
            case "crazy":
                return new CrazyLid(idCounter, size, color, centerX, FLOOR_Y, size);
            default:
                return null;
        }
    }
